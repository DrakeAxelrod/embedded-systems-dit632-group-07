...
#endif
// End of declaration of the TimerOne library

float temp = 0; // create a variable for the temperature
int limits[6] = {0, 10, 30, 60, 100}; // create an integer array that holds the temperature limits in the ranges
int pins[5] = {0, 0, 0, 0, 0}; // create an integer array and initialise all elements to 0

void which_LEDS_to_switch_on(); // definition of function

void setup() // setup function runs once in the beginning of the loop
{
  pinMode(A0, INPUT); // set pin A0 as an input from the temperature sensor
  pinMode(7, OUTPUT); // set pin 7 as an output to an LED
  pinMode(6, OUTPUT); // set pin 7 as an output to an LED
  pinMode(5, OUTPUT); // set pin 7 as an output to an LED
...
{
  int temperature = analogRead(A0); // get the analog temperature from the pin
...
void which_LEDS_to_switch_on() // function to check which LEDs should be switched on
{
  ...
  {
    if (temp >= limits[i]) // if the temperature is higher than the current limit
    {
      digitalWrite(pin, HIGH); // switch the LED on
    }
    pin--; // decrement pin int
  }
}
