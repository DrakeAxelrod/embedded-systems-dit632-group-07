...
void setup() {
  ...
  pinMode(PWM2,OUTPUT);
  // ========== Added ========== //
  attachInterrupt(digitalPinToInterrupt(ENCA), ISR_readEncoder, RISING); // this interrupt is on b
  // =========================== //
...
int getAction(int error){
  // ========== Added ========== //
  u_out = kp * error;            // given algorithm from wp6ex1
  // =========================== //
...
void ISR_readEncoder(){
  // ========== Added ========== //
  b = digitalRead(ENCB);         // read channel B
  (b > 0) ? pos-- : pos++;       // we read a since we attach to chanB then we inc or dec depending on low or high
  // =========================== //
}
